...
def connect_to_dropbox(access_token=ACCESS_TOKEN):
    try:
...
def download_file_from_dropbox(dropbox_object, local_file_path=LOCAL_DOWNLOAD_FILE_PATH,
...
def upload_file_to_dropbox(dropbox_object, local_file_path, dropbox_file_path):
    try:
...
def anonymize(file_path=LOCAL_DOWNLOAD_FILE_PATH):
...
download_file_from_dropbox(dropbox_object=dbx)
anonymize()
upload_file_to_dropbox(dbx, LOCAL_UPLOAD_FILE_PATH, DROPBOX_UPLOAD_FILE_PATH)
upload_file_to_dropbox(dbx, LOCAL_UPLOAD_OFFSET_FILE_PATH, DROPBOX_OFFSET_FILE_PATH)
